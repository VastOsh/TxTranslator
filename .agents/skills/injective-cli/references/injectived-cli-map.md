...
- `query`: Querying subcommands
  ...
  - `hyperlane`: Querying commands for the hyperlane module
    - `delivered`: Check if a message has been delivered
    - `hooks`: Querying commands for the post-dispatch module
      - `destination-gas-configs`: List destination gas configs for an IGP
      - `igp`: Get details for a specific interchain gas paymaster
      - `igps`: List all interchain gas paymasters
      - `merkle-tree-hook`: Get details for a specific merkle tree hook
      - `merkle-tree-hooks`: List all merkle tree hooks
      - `noop-hook`: Get details for a specific noop hook
      - `noop-hooks`: List all noop hooks
      - `quote-gas-payment`: Quote gas payment for a transaction
    - `ism`: Querying commands for the ism module
      - `announced-storage-locations`: Query announced storage locations for a validator
      - `ism`: Get ISM details by ID
      - `isms`: List all ISMs
      - `latest-announced-storage-location`: Query the latest announced storage location for a validator
    - `mailbox`: Get mailbox details
    - `mailboxes`: List all mailboxes
    - `recipient-ism`: Get ISM ID for a recipient
    - `registered-apps`: List all registered applications
    - `registered-hooks`: List all registered hooks
    - `registered-isms`: List all registered ISMs
    - `verify-dry-run`: Perform a dry run verification of a message
  ...
  - `warp`: Querying commands for the warp module
    - `bridged-supply`: Query bridged supply of a token
    - `quote-transfer`: Quote gas payment for a remote transfer
    - `remote-routers`: Query remote routers for a token
  ...
  - `xwasm`: Querying commands for the xwasm module
    - `contract-info`: Gets contract info
...
- `tx`: Transactions subcommands
  ...
  - `chainlink`: OCR subcommands
    - `create-feed`: Create feed
    - `fund-feed-reward-pool`: Fund feed reward pool
    - `set-config-proposal`: Submit a proposal to set config.
  ...
  - `hyperlane`: hyperlane transactions subcommands
    - `hooks`: Hyperlane Core-Hooks commands
      - `igp`: Hyperlane Interchain Gas Paymaster commands
        - `claim`: Claim Hyperlane Interchain Gas Paymaster fees
        - `create`: Create a Hyperlane Interchain Gas Paymaster
        - `pay-for-gas`: Hyperlane Interchain Gas Payment without using QuoteGasPayment
        - `set-destination-gas-config`: Set Destination Gas Config for Interchain Gas Paymaster
        - `set-owner`: Update a Hyperlane Interchain Gas Paymaster
      - `merkle`: Hyperlane Merkle Tree Hook commands
        - `create`: Create a new merkle tree hook with the given mailbox id
      - `noop`: Hyperlane Noop Hook commands
        - `create`: Create a new noop hook
    - `ism`: Hyperlane Interchain Security Module commands
      - `announce`: Announce a Hyperlane validator
      - `create-merkle-root-multisig`: Create a Hyperlane MerkleRoot Multisig ISM
      - `create-message-id-multisig`: Create a Hyperlane MessageId Multisig ISM
      - `create-noop`: Create a Hyperlane Noop ISM
      - `create-routing`: Create a Hyperlane Routing ISM
      - `remove-routing-ism-domain`: Removes the ISM for a given domain in the routing ISM
      - `set-routing-ism-domain`: Sets the ISM for a given domain in the routing ISM
      - `update-routing-ism-owner`: Update the owner of a routing ISM
    - `mailbox`: Hyperlane Mailbox commands
      - `create`: Create a Hyperlane Mailbox
      - `process`: Process a Hyperlane message
      - `set`: Update a Hyperlane Mailbox
  ...
  - `warp`: warp transactions subcommands
    - `create-collateral-token`: Create a Hyperlane Collateral Token
    - `create-synthetic-token`: Create a Hyperlane Synthetic Token
    - `enroll-remote-router`: Enroll remote router for a certain token
    - `set-token`: Update the Warp token
    - `transfer`: Send Hyperlane Token
    - `unroll-remote-router`: Unroll remote router for a certain token
