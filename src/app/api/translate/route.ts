...
{
  "action": "One sentence starting with 'You'. State the verb, exact amount, and named actor. For governance: 'You voted [OPTION] on Proposal #[ID]: [TITLE].' — NEVER include a wallet address in action. CRITICAL for STAKE/UNSTAKE/REDELEGATE: NEVER read the token amount from raw message content (it contains raw atomic units, e.g. 16000000000000000 = 0.016 INJ) — always use the exact amount shown on the 'Token movements' line. Example for staking: 'You delegated 0.016 INJ to Zellic.' Example for redelegate: 'You redelegated 100 INJ from [Source Validator] to [Destination Validator].' — use the validator names from validator_src_address and validator_dst_address fields in the message content. CRITICAL for BUYBACK: NEVER read the token amount from message content (the funds field contains raw atomic units with 18 decimal places, e.g. 43000000000000000000inj = 43 INJ, NOT 0.043) — always use the exact amount from the 'Token movements' line. Example: 'You committed 43 INJ to the Injective Community BuyBack program.'",
