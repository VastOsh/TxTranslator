...
import type { NormalizedTransaction, MultiSendRecipient, TradeData, UnbondingData, GovernanceData } from '@/types';
...
${messagesBlock}`;
  ...
  if (liveVP != null || liveCommission != null || validatorLiveInfo != null || networkAPR != null) {
    ...
    if (networkAPR != null) {
      const baseAPR = networkAPR.toFixed(2);
      prompt += `\n  ⚠ Live network base APR (from chain, NOT hardcoded): ${baseAPR}% — use this value verbatim`;
      if (liveCommission != null) {
        const effectiveAPR = (networkAPR * (1 - liveCommission / 100)).toFixed(1);
        prompt += `\n  Effective delegator APR (${baseAPR}% × (1 − ${liveCommission.toFixed(1)}%)): ~${effectiveAPR}% — copy verbatim`;
      }
    }
  }
  ...
  if (tx.tradeData) {
    ...
    const orderType = td.isLimitOrder ? 'LIMIT (maker)' : 'MARKET (taker)';
    const feeStr = td.feeAmount != null ? `${td.feeAmount} ${td.feeSymbol}` : 'N/A';
    const feeContext = buildTradeFeeContext(td);
  ...
  Received: ${td.receivedAmount ?? '?'} ${td.receivedSymbol ?? ''}
  Execution price (backend-calculated = received ÷ spent): ${td.executionPrice ?? '?'} ${td.quoteSymbol ?? ''}/${td.baseSymbol ?? ''}
  Slippage (backend-calculated = |actual − orderBook| ÷ orderBook): ${td.slippagePct != null ? `${td.slippagePct}%` : 'N/A'}
