...
// Adapted from the React Bits "Ferrofluid" component (ogl WebGL shader).
// Renzu changes: tracks the global cursor (spike follows even over the hero
// content, and the canvas never intercepts clicks), fails gracefully when
// WebGL is unavailable, and carries its own container styles.
...
export default function Ferrofluid({
  ...
  useEffect(() => {
    ...
    try {
      ...
      return; // WebGL unavailable — leave the plain background in place
    }
    ...
    // Track the GLOBAL cursor so the magnetic spike follows even over hero
    // content; the canvas itself stays pointer-events:none (set in CSS).
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      mouseTarget[0] = x; mouseTarget[1] = y;
    ...
    const loop = (t: number) => {
      ...
      if (pausedRef.current) { lastTime = t; return; }
    ...
    dpr, colors, speed, scale, turbulence, fluidity, rimWidth, sharpness, shimmer,
    glow, flowDirection, opacity, mouseInteraction, mouseStrength, mouseRadius, mouseDampening,
  ]);
